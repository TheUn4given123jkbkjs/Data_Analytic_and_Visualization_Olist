...
# BASE_DIR là thư mục chứa app.py (thư mục 05_Dashboard)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# PROJECT_ROOT là thư mục cha chứa tất cả các thư mục 01_..., 02_..., 05_...
...
def find_image_path(filename: str) -> str | None:
    """
    Tự động tìm kiếm đường dẫn tương đối hoặc tuyệt đối của file ảnh
    trong toàn bộ cấu trúc dự án (PROJECT_ROOT).
    """
    # 1. Trường hợp filename truyền vào đã có chứa thư mục con (VD: "01_Descriptive/aov_distribution.png")
    full_path = os.path.join(PROJECT_ROOT, filename)
    if os.path.exists(full_path):
        return full_path

    # 2. Trường hợp filename chỉ là tên file (VD: "aov_distribution.png")
    # Duyệt qua toàn bộ thư mục con trong PROJECT_ROOT để tìm file
...
def render_chart(chart: dict):
    ...
    if img_path and os.path.exists(img_path):
        ...
        st.warning(f"⚠️ Chưa tìm thấy ảnh: `{chart['file']}")
